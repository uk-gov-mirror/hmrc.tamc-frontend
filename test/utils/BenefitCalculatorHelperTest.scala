  ...
  "BenefitCalculatorHelper" when {
    ...
    "calculateTotalBenefitAcrossBands" must {
      ...
      "return the correct total benefit for an English tax payer" in {
        ...
        benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          applicationConfig.PERSONAL_ALLOWANCE() + (applicationConfig
            .MAX_ALLOWED_PERSONAL_ALLOWANCE_TRANSFER() * 0.5) - applicationConfig.PERSONAL_ALLOWANCE(),
          eligibilityCalculatorService.getCountryTaxBands(England, currentTaxYear)
        ) shouldBe (applicationConfig.MAX_BENEFIT() * 0.5).floor.toInt

        benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          applicationConfig.PERSONAL_ALLOWANCE() + (applicationConfig
            .MAX_ALLOWED_PERSONAL_ALLOWANCE_TRANSFER() * 0.8) - applicationConfig.PERSONAL_ALLOWANCE(),
          eligibilityCalculatorService.getCountryTaxBands(England, currentTaxYear)
      ...
      "return the correct total benefit for a Scottish tax payer" in {
        ...
        benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          13150 - applicationConfig.PERSONAL_ALLOWANCE(),
          eligibilityCalculatorService.getCountryTaxBands(Scotland, currentTaxYear)
        ) shouldBe 110

        benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          13650 - applicationConfig.PERSONAL_ALLOWANCE(),
          eligibilityCalculatorService.getCountryTaxBands(Scotland, currentTaxYear)
        ) shouldBe 205
      }
      ...
      "return the correct total benefit for a Scottish tax payer when hypothetical bands are used" in {
        val hypotheticalBandsOne = List(
          TaxBand("StarterRate", 11851, 12050, 0.19),
          TaxBand("BasicRate", 12051, 24000, 0.20),
          TaxBand("IntermediateRate", 24001, 43430, 0.21)
        )

        benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          13150 - applicationConfig.PERSONAL_ALLOWANCE(),
          hypotheticalBandsOne
        ) shouldBe 114

        val hypotheticalBandsTwo = List(
          TaxBand("StarterRate", 11851, 13850, 0.10),
          TaxBand("BasicRate", 13851, 24000, 0.21),
          TaxBand("IntermediateRate", 24001, 43430, 0.22)
        )

        benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          13150 - applicationConfig.PERSONAL_ALLOWANCE(),
          hypotheticalBandsTwo
        ) shouldBe 58

        val hypotheticalBandsThree = List(
          TaxBand("StarterRate", 11851, 12050, 0.10),
          TaxBand("BasicRate", 12051, 22050, 0.15),
          TaxBand("FakeRate1", 22051, 27050, 0.20),
          TaxBand("FakeRate2", 27051, 32050, 0.25)
        )

        val personOne = benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          30650 - applicationConfig.PERSONAL_ALLOWANCE(),
          hypotheticalBandsThree
        )
        val personTwo = benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          11650 - applicationConfig.TRANSFEROR_ALLOWANCE,
          hypotheticalBandsThree
        )
        personOne - personTwo shouldBe 148

        val personThree = benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          13150 - applicationConfig.PERSONAL_ALLOWANCE(),
          hypotheticalBandsThree
        )
        val personFour  = benefitCalculatorHelper.calculateTotalBenefitAcrossBands(
          11650 - applicationConfig.TRANSFEROR_ALLOWANCE,
          hypotheticalBandsThree
        )
    ...
    "getCountryTaxBandsFromFile is called" must {
      ...
      "return the correct banded income for a Scottish tax payer" in {
        ...
        val starterRate      =
          applicationConfig.getTaxBand("StarterRate", "starter-rate", "scotland", currentTaxYear.startYear)
        val basicRate        = applicationConfig.getTaxBand("BasicRate", "basic-rate", "scotland", currentTaxYear.startYear)
        val intermediateRate =
          applicationConfig.getTaxBand("IntermediateRate", "intermediate-rate", "scotland", currentTaxYear.startYear)
        ...
        eligibilityCalculatorService.getCountryTaxBands(Scotland, currentTaxYear) shouldBe
          List(
            TaxBand(
              ...
              starterRate.upperThreshold,
              starterRate.rate
            ),
            basicRate,
            TaxBand(
              "IntermediateRate",
              intermediateRate.lowerThreshold,
              applicationConfig.MAX_LIMIT_SCOT(),
              intermediateRate.rate
            )
          )
      }
      ...
      "return the correct banded income for a Welsh tax payer" in {
        eligibilityCalculatorService.getCountryTaxBands(Wales, currentTaxYear) shouldBe
          List(
            TaxBand("BasicRate", applicationConfig.PERSONAL_ALLOWANCE() + 1, applicationConfig.MAX_LIMIT_WALES(), 0.20)
          )
      }
        ...
        eligibilityCalculatorService.getCountryTaxBands(NorthernIreland, currentTaxYear) shouldBe
          List(
            TaxBand(
              "BasicRate",
              applicationConfig.PERSONAL_ALLOWANCE() + 1,
              applicationConfig.MAX_LIMIT_NORTHERN_IRELAND(),
              0.20
            )
          )
      }
