...
class ClaimsViewModelTest extends BaseTest {
  ...
  lazy val formattedEndOfTaxYear: String =
    endOfTaxYear.format(DateTimeFormatter.ofPattern("d MMMM yyyy").withLocale(Locale.UK))
  ...
  def createRelationshipRecord(
    creationTimeStamp: LocalDate = now.minusDays(1),
    ...
    otherParticipantUpdateTimestamp: LocalDate = now.minusDays(1)
  ): RelationshipRecord =
    RelationshipRecord(
      Transferor.value,
  ...
  "ClaimsViewModel" should {
    ...
    "create a ClaimsViewModel with historic claims rows" when {
      ...
      lazy val dateInterval     = languageUtils.taxDateIntervalString(
        participant1StartDate.format(DateTimeFormatter.ofPattern(dateInputPattern)),
      ...
      endReasons.foreach { endReason =>
        s"endRelationship reason is $endReason" in {
          ...
          val historicNonPrimaryRecord = createRelationshipRecord(
            creationTimeStamp,
            participant1StartDate,
            Some(endReason),
            Some(participant1EndDate)
          )
    ...
    "create a ClaimsViewModel which has an ordered historic non primary sequence of claims" in {
      ...
      val previousClaimRecordMinus1 = createRelationshipRecord(
        creationTimeStamp = previousClaims1CreationDate,
        participant1StartDate = previousClaims1StartDate,
        participant1EndDate = Some(previousClaims1EndDate),
        relationshipEndReason = Some(Divorce)
      )

      val previousClaimRecordMinus2 = createRelationshipRecord(
        creationTimeStamp = previousClaims2CreationDate,
        participant1StartDate = previousClaims2StartDate,
        participant1EndDate = Some(previousClaims2EndDate),
        relationshipEndReason = Some(Cancelled)
      )

      val previousClaimRecordMinus3 = createRelationshipRecord(
        creationTimeStamp = previousClaims3CreationDate,
        participant1StartDate = previousClaims3StartDate,
        participant1EndDate = Some(previousClaims3EndDate)
      )

      val historicSequenceUnordered =
        Seq(previousClaimRecordMinus2, previousClaimRecordMinus1, previousClaimRecordMinus3)

      val expectedClaimsRow1 =
        createExpectedClaimRow(previousClaims1StartDate, previousClaims1EndDate, previousClaimRecordMinus1)
      val expectedClaimsRow2 =
        createExpectedClaimRow(previousClaims2StartDate, previousClaims2EndDate, previousClaimRecordMinus2)
      val expectedClaimsRow3 =
        createExpectedClaimRow(previousClaims3StartDate, previousClaims3EndDate, previousClaimRecordMinus3)
    ...
    "create a historic claims row" when {
      "there is no historic end reason set" in {
        ...
        val cyMinusOneRecord = createRelationshipRecord(
          participant1StartDate = cyMinusOneStartDate,
          participant1EndDate = Some(cyMinusOneEndDate),
  ...
  private def getDateRange(startDate: LocalDate, endDate: LocalDate) =
    languageUtils.taxDateIntervalString(
      startDate.format(DateTimeFormatter.ofPattern(dateInputPattern)),
