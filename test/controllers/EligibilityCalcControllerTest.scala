...
class EligibilityCalcControllerTest extends BaseTest {
  ...
  "Check validation message" should {
    ...
    "return 400 when form is submitted with a single error" in {
      val request =
        FakeRequest().withMethod("POST").withFormUrlEncodedBody("country" -> "england", "transferor-income" -> "9000")
      val result  = controller.gdsCalculatorAction()(request)
    ...
    "return 400 if transferor income is not provided (None)" in {
      val request =
        FakeRequest().withMethod("POST").withFormUrlEncodedBody("country" -> "england", "recipient-income" -> "5000")
      val result  = controller.gdsCalculatorAction()(request)
      status(result) shouldBe BAD_REQUEST
    }
    ...
    "return 400 if transferor income contains letters" in {
      val request  = FakeRequest()
        .withMethod("POST")
        .withFormUrlEncodedBody("country" -> "england", "transferor-income" -> "abc", "recipient-income" -> "5000")
      val result   = controller.gdsCalculatorAction()(request)
      val document = Jsoup.parse(contentAsString(result))
      val form     = document.getElementById("calculator")
      form
        .getElementById("transferor-income-error")
        .text() shouldBe "Error: Your income (low), before tax is taken off must only include numbers 0 to 9"
    }

    "return 400 if transferor income contains negative number" in {
      val request = FakeRequest()
        .withMethod("POST")
        .withFormUrlEncodedBody("country" -> "england", "transferor-income" -> "-1", "recipient-income" -> "5000")
      val result  = controller.gdsCalculatorAction()(request)
      status(result) shouldBe BAD_REQUEST
    }
    ...
    "return 400 if recipient income is not provided (None)" in {
      val request =
        FakeRequest().withMethod("POST").withFormUrlEncodedBody("country" -> "england", "transferor-income" -> "5000")
      val result  = controller.gdsCalculatorAction()(request)
      status(result) shouldBe BAD_REQUEST
    }
    ...
    "return 400 if recipient income contains letters" in {
      val request = FakeRequest()
        .withMethod("POST")
        .withFormUrlEncodedBody("country" -> "england", "recipient-income" -> "abc", "transferor-income" -> "5000")
      val result  = controller.gdsCalculatorAction()(request)
      status(result) shouldBe BAD_REQUEST
    }
    ...
    "return 400 if recipient income contains negative number" in {
      val request = FakeRequest()
        .withMethod("POST")
        .withFormUrlEncodedBody("country" -> "england", "recipient-income" -> "-1", "transferor-income" -> "5000")
      val result  = controller.gdsCalculatorAction()(request)
      status(result) shouldBe BAD_REQUEST
    }
