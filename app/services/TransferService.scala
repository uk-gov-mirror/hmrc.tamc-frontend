...
class TransferService @Inject() (
  ...
  def isRecipientEligible(transferorNino: Nino, recipientData: RegistrationFormInput)(implicit
    request: Request[?],
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Boolean] =
    checkRecipientEligible(transferorNino, recipientData).map(eligible => eligible) recoverWith { case error =>
      handleAudit(RecipientFailureEvent(transferorNino, error))
      Future.failed(error)
    }

  private def checkRecipientEligible(transferorNino: Nino, recipientData: RegistrationFormInput)(implicit
    request: Request[?],
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[Boolean] =
    for {
      ...
      _                      <- cachingService.put[RecipientRecord](
                                  CACHE_RECIPIENT_RECORD,
                                  RecipientRecord(record = userRecord, data = recipientData, availableTaxYears = validYears)
                                )
    } yield true
  ...
  private def validateTransferorAgainstRecipient(
    recipientData: RegistrationFormInput,
    cache: Option[EligibilityCheckCacheData]
  ): Future[Option[EligibilityCheckCacheData]] =
    (recipientData, cache) match {
      case (
            RegistrationFormInput(_, _, _, _, dom),
            Some(EligibilityCheckCacheData(_, _, activeRelationshipRecord, historicRelationships, _, _, _))
          )
          if applicationService.canApplyForMarriageAllowance(
            historicRelationships,
            activeRelationshipRecord,
            timeService.getTaxYearForDate(dom)
          ) =>
  ...
  def createRelationship(transferorNino: Nino)(implicit
    request: Request[?],
    hc: HeaderCarrier,
    messages: Messages,
    ec: ExecutionContext
  ): Future[NotificationRecord] =
    doCreateRelationship(transferorNino)(request, hc, messages, ec) recover { case error =>
  ...
  private def validateFinishedData(
    cacheData: Option[UserAnswersCacheData]
  )(implicit ec: ExecutionContext): Future[NotificationRecord] =
    Future {
      cacheData match {
  ...
  private def doCreateRelationship(transferorNino: Nino)(implicit
    request: Request[?],
    hc: HeaderCarrier,
    messages: Messages,
    ec: ExecutionContext
  ...
  def getRecipientDetailsFormData()(implicit
    request: Request[?],
    ec: ExecutionContext
  ): Future[RecipientDetailsFormInput] =
    for {
      userAnswersCachedData <- getUserAnswersCachedData
  ...
  private def validateRegistrationData(
    cacheData: Option[UserAnswersCacheData]
  )(implicit ec: ExecutionContext): Future[RecipientDetailsFormInput] =
    Future {
      cacheData match {
  ...
  private def auditCreateRelationship(
    cacheData: UserAnswersCacheData
  )(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Unit] =
    handleAudit(CreateRelationshipSuccessEvent(cacheData))

  def upsertTransferorNotification(
    notificationRecord: NotificationRecord
  )(implicit request: Request[?]): Future[NotificationRecord] = {
  ...
  def getConfirmationData(
    nino: Nino
  )(implicit request: Request[?], hc: HeaderCarrier, ec: ExecutionContext): Future[ConfirmationModel] = {
  ...
  def getUserAnswersCachedDataWithTransferorRecord(
    nino: Nino
  )(implicit request: Request[?], hc: HeaderCarrier, ec: ExecutionContext): Future[Option[UserAnswersCacheData]] =
  ...
  private def updateCacheWithTransferorRecord(
    nino: Nino
  )(implicit request: Request[?], hc: HeaderCarrier, executionContext: ExecutionContext): Future[Unit] =
  ...
  private def getTransferorRecord(
    nino: Nino
  )(implicit hc: HeaderCarrier, executionContext: ExecutionContext): Future[UserRecord] = marriageAllowanceConnector
  ...
  private def validateCompleteCache(
    cacheData: Option[UserAnswersCacheData]
  )(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[UserAnswersCacheData] = {
    ...
    cacheData match {
      ...
      case Some(UserAnswersCacheData(_, _, _, _, Some(selectedTaxYears), _, _)) if selectedTaxYears.isEmpty =>
        throw NoTaxYearsSelected()
      case _                                                                                                => throw BadFetchRequest()
    }
  }

  private def transformCache(
    cacheData: UserAnswersCacheData
  )(implicit ec: ExecutionContext): Future[ConfirmationModel] =
    Future {
      ConfirmationModel(
  ...
  private def getRecipientRelationship(transferorNino: Nino, recipientData: RegistrationFormInput)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext
  ): Future[(UserRecord, Option[List[TaxYear]])] =
      ...
      .flatMap {
        ...
        case Left(error)                    =>
          (error.status, error.statusCode, error.message) match {
            ...
            case (_, Some(statusCode), Some(message))
                if statusCode == 400 && message.contains("Participant is deceased") =>
              Future.failed(TransferorDeceased())
            case _                            => Future.failed(OtherError(error))
          }
      }

  def deleteSelectionAndGetCurrentAndPreviousYearsEligibility(implicit
    request: Request[?],
    ec: ExecutionContext
  ): Future[CurrentAndPreviousYearsEligibility] =
    for {
      _   <- saveSelectedYears(List[Int]())
      res <- getCurrentAndPreviousYearsEligibility
    } yield res

  def getCurrentAndPreviousYearsEligibility(implicit
    request: Request[?],
    ec: ExecutionContext
  ): Future[CurrentAndPreviousYearsEligibility] =
  ...
  private def transform(sessionData: UserAnswersCacheData, messages: Messages): CreateRelationshipRequestHolder = {
    ...
    val sendNotificationData   = CreateRelationshipNotificationRequest(
      full_name = "UNKNOWN",
      email = email,
      welsh = languageUtilsImpl.isWelsh(messages)
    )
    CreateRelationshipRequestHolder(request = createRelationshipreq, notification = sendNotificationData)
  }

  private def sendCreateRelationship(transferorNino: Nino, data: UserAnswersCacheData)(implicit
    hc: HeaderCarrier,
    messages: Messages,
    ec: ExecutionContext
  ): Future[UserAnswersCacheData] =
  ...
  def updateSelectedYears(availableTaxYears: List[TaxYear], extraYear: Int, yearAvailableForSelection: Option[Int])(
    implicit
    request: Request[?],
    ec: ExecutionContext
  ): Future[List[Int]] =
  ...
  def updateSelectedYears(
    availableTaxYears: List[TaxYear],
    extraYears: List[Int],
    yearAvailableForSelection: Option[Int]
  )(implicit request: Request[?], ec: ExecutionContext): Future[List[Int]] =
    for {
      userAnswersCachedData  <- getUserAnswersCachedData
  ...
  def validateSelectedYears(
    availableTaxYears: List[TaxYear],
    selectedYears: List[Int],
    yearAvailableForSelection: Option[Int]
  )(implicit ec: ExecutionContext): Future[List[Int]] =
    if (
      selectedYears.isEmpty && yearAvailableForSelection.isDefined && availableTaxYears.nonEmpty && yearAvailableForSelection.get == availableTaxYears.last.year
    ) {
  ...
  private def updateSelectedYears(cy: Option[List[Int]], extraYears: List[Int])(implicit
    ec: ExecutionContext
  ): Future[List[Int]] =
    Future {
