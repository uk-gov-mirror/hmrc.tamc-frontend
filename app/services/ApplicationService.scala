...
class ApplicationService @Inject() (
  ...
  def canApplyForMarriageAllowance(
    ...
    startingFromTaxYear: Int = timeService.getEarliestValidYear()
  ): Boolean =
  ...
  def canApplyForCurrentYears(
    ...
    !taxYearsThatAreUnavailableForApplication(historicRelationships, activeRelationship).contains(
      timeService.getCurrentTaxYear
    )
  ...
  private def taxYearsThatAreUnavailableForApplication(
    historicRelationships: Option[Seq[RelationshipRecord]],
    activeRelationship: Option[RelationshipRecord]
  ): Set[Int] = {
    val historicYears: Set[Set[Int]] = historicRelationships
      .getOrElse(Seq[RelationshipRecord]())
      .toSet
      .filter { relationship =>
    ...
    val activeYears: Set[Int]   = activeRelationship
      .map {
        _.overlappingTaxYears(appConfig.currentTaxYear().startYear)
      }
      .getOrElse(Set[Int]())
