...
class ApplicationConfig @Inject() (configuration: Configuration, servicesConfig: ServicesConfig) {
  ...
  def currentTaxYear(): TaxYear = TaxYear(
    configuration.getOptional[Int]("tamc-effective-tax-year").getOrElse(TaxYear.current.startYear)
  )

  def currentLocalDate(): LocalDate =
    LocalDate.parse(configuration.getOptional[String]("tamc-effective-local-date").getOrElse(LocalDate.now.toString))
  ...
  lazy val betaFeedbackUnauthenticatedUrl =
    s"$contactHost/beta-feedback-unauthenticated?service=$contactFormServiceIdentifier"
  ...
  lazy val ggSignInHost: String                =
    configuration.getOptional[String]("microservice.bas-gateway-frontend.host").getOrElse("")
  lazy val ggSignInUrl: String                 =
    s"$ggSignInHost/bas-gateway/sign-in?continue_url=${encodeQueryStringValue(callbackUrl)}"

  lazy val marriageAllowanceUrl: String = servicesConfig.baseUrl("marriage-allowance")
  lazy val taiFrontendUrl: String       =
    s"${configuration.getOptional[String]("microservice.tai-frontend.host").getOrElse("")}/check-income-tax"
  lazy val taxFreeAllowanceUrl          = s"$taiFrontendUrl/tax-free-allowance"

  val DEFAULT_VALID_YEARS   = 4
  val TAMC_VALID_YEARS: Int = configuration
    .getOptional[Int]("tamc-valid-years-prior")
  ...
  def PERSONAL_ALLOWANCE(taxYear: Int = 0): Int                      =
    configuration.getOptional[Int]("personal-allowance-" + actualTaxYear(taxYear)).getOrElse(0)
  def MAX_LIMIT(taxYear: Int = 0): Int                               =
    configuration.getOptional[Int]("max-limit-" + actualTaxYear(taxYear)).getOrElse(0)
  def MAX_LIMIT_SCOT(taxYear: Int = 0): Int                          =
    configuration.getOptional[Int]("max-limit-scot-" + actualTaxYear(taxYear)).getOrElse(0)
  def MAX_LIMIT_WALES(taxYear: Int = 0): Int                         =
    configuration.getOptional[Int]("max-limit-wales-" + actualTaxYear(taxYear)).getOrElse(0)
  def MAX_LIMIT_NORTHERN_IRELAND(taxYear: Int = 0): Int              =
    configuration.getOptional[Int]("max-limit-northern-ireland-" + actualTaxYear(taxYear)).getOrElse(0)
  def MAX_ALLOWED_PERSONAL_ALLOWANCE_TRANSFER(taxYear: Int = 0): Int =
    configuration.getOptional[Int]("max-allowed-personal-allowance-transfer-" + actualTaxYear(taxYear)).getOrElse(0)
  def MAX_BENEFIT(taxYear: Int = 0): Int                             =
    configuration.getOptional[Int]("max-benefit-" + actualTaxYear(taxYear)).getOrElse(0)

  def STARTER_RATE_LOWER_TRESHOLD_SCOT(taxYear: Int = 0): Int = configuration
    .getOptional[Int]("taxbands-starter-rate-lowerthreshold-scotland-" + actualTaxYear(taxYear))
    .getOrElse(0)
  ...
  val accessibilityStatementHost: String                              =
    loadConfig("accessibility-statement.service-path") + "/accessibility-statement"
  def accessibilityStatementUrl(relativeReferrerPath: String): String =
    accessibilityStatementHost + "/marriage-allowance?referrerUrl=" + encodeQueryStringValue(
  ...
  def getTaxBand(name: String, taxRate: String, country: String, taxYear: Int): TaxBand = {
    val lowerThreshold =
      configuration.getOptional[Int](taxBandKey(taxRate, "lowerthreshold", country, taxYear)).getOrElse(0)
    val upperThreshold =
      configuration.getOptional[Int](taxBandKey(taxRate, "upperthreshold", country, taxYear)).getOrElse(0)
