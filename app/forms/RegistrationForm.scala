...
class RegistrationForm @Inject() (applicationConfig: ApplicationConfig) {
  ...
  private def maxLengthWithError(maxLength: Int, error: String): Constraint[String] =
    Constraint[String]("constraint.maxLength", maxLength) { o =>
      if (o == null) Invalid(ValidationError(error, maxLength))
      else if (o.length <= maxLength) Valid
  ...
  private def ninoMapping(
    ...
    nonEmptyTrimmer(error = errorRequired)
      ...
      .verifying(
        ninoIsValidFormat(formatError = errorInvalid, charError = errorInvalidChars, maxLengthError = errorMaxLength)
      )
      .transform[Nino](Nino(_), _.nino)

  private def ninoIsValidFormat(
    name: String = "constraint.ninoFormat",
    formatError: String,
    charError: String,
    ...
    Constraint[String](name) { nino =>
      if (Nino.isValid(nino)) Valid
      else {
  ...
  def dateOfMarriageValidator(today: LocalDate)(implicit messages: Messages): Mapping[LocalDate] = {
    ...
    validDateTuple()
      ...
      .verifying(
        error =
          Messages("pages.form.field.dom.error.min-date", minDate.format(DateTimeFormatter.ofPattern("d MM YYYY"))),
        constraint = _.isAfter(minDate)
      )
      .verifying(error = Messages("pages.form.field.dom.error.max-date"), constraint = _.isBefore(maxDate))
  }

  def registrationForm(today: LocalDate, transferorNino: Nino)(implicit messages: Messages) =
    Form[RegistrationFormInput](
      mapping(
        ...
        "gender"         -> gender,
        "nino"           -> nino.verifying(
          "pages.form.field.nino.error.self",
          recipientNino => !utils.areEqual(transferorNino, recipientNino)
        ),
        "dateOfMarriage" -> dateOfMarriageValidator(today)
      )(RegistrationFormInput.apply)(RegistrationFormInput.unapply)
    )
}
