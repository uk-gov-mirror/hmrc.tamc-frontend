...
object EarlierYearForm {
  ...
  def earlierYearsForm(furtherYears: List[Int] = List()): Form[EarlierYearsInput] = Form[EarlierYearsInput](
    mapping("selectedYear" -> number, "furtherYears" -> text, "yearAvailableForSelection" -> optional(number))(
      earlierYearsFormToEntity
    )(entityToEarlierYearsForm)
  )

  private def earlierYearsFormToEntity(
    selectedYear: Int,
    furtherYears: String,
