...
object PlayFormFormatter {
  ...
  /** Defines a maximum length constraint for [[EmailAddress]] values
    *
    * @param error
    *   error message with default value `"error.maxLength(maxLength)"`
    * @param name
    *   constraint's name with default value `"constraint.maxLength(maxLength)"`
    */
  def emailMaxLength(
    maxLength: Int,
    name: String = "constraint.maxLength",
    error: String = "error.maxLength"
  ): Constraint[EmailAddress] =
    Constraint[EmailAddress](name, maxLength) { email =>
  ...
  def validDateTuple(
    missingYearError: String = "pages.form.field.dom.error.must.include.year",
    ...
    tuple(
      ...
      .transform[(Int, Int, Int)](
        x => (x._1.toInt, x._2.toInt, x._3.toInt),
        x => (x.toString(), x.toString(), x.toString())
      )
      .verifying(
        checkDateRangeValidator(
          ...
          yearTodayOrPast = yearTodayOrPast
        )
      )
      .transform[ZonedDateTime](
        x => ZonedDateTime.of(x._1, x._2, x._3, 0, 0, 0, 0, ZoneId.systemDefault()),
        x => (x.getYear, x.getMonthValue, x.getDayOfMonth)
      )
  }

  private def datePartsArePresent(
  ...
  ): Constraint[(Option[String], Option[String], Option[String])] =
  ...
  private def checkDateRangeValidator(
    ...
    Constraint[(Int, Int, Int)](name) {
      ...
      case tup if tup._3 < 1 || tup._3 > dayRange(tup._2, tup._1) =>
        Invalid(ValidationError(invalidDay, dayRange(tup._2, tup._1)))
      case tup
          if dayRange(tup._2, tup._1) <= 31 && tup._2 <= 12 && tup._1.toString.length == 4 && tup._1 > currentYear =>
        Invalid(ValidationError(yearTodayOrPast))
