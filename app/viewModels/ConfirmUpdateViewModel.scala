...
class ConfirmUpdateViewModelImpl @Inject() (languageUtilsImpl: LanguageUtilsImpl) {
  ...
  def apply(updateAnswers: ConfirmationUpdateAnswers)(implicit messages: Messages): ConfirmUpdateViewModel = {
    ...
    val nameRow       = SummaryRow(
      messages("pages.confirm.cancel.your-name"),
      "change-link-name",
      "change name",
      updateAnswers.loggedInUserInfo.name.flatMap(_.fullName).getOrElse(""),
      None,
      1
    )
    val emailRow      = SummaryRow(
      messages("pages.confirm.cancel.email"),
      "change-link-email",
      "change email",
      updateAnswers.email,
      Some(controllers.UpdateRelationship.routes.ConfirmEmailController.confirmEmail().url),
      3
    )
    ...
    val effectiveDate =
      languageUtilsImpl().ukDateTransformer(updateAnswers.maEndingDates.personalAllowanceEffectiveDate)
    ...
    def createRows(defaultRows: List[SummaryRow])(implicit messages: Messages): List[SummaryRow] =
      updateAnswers.divorceDate.fold(defaultRows) { divorceDate =>
        ...
        SummaryRow(
          ...
          "change date",
          formattedDate,
          Some(controllers.UpdateRelationship.routes.DivorceController.divorceEnterYear().url),
          2
        ) :: defaultRows
      }
