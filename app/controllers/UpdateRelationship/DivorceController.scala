...
class DivorceController @Inject() (
  ...
  def divorceEnterYear: Action[AnyContent] = authenticate.pertaxAuthActionWithUserDetails.async { implicit request =>
  ...
  def submitDivorceEnterYear: Action[AnyContent] = authenticate.pertaxAuthActionWithUserDetails.async {
    implicit request =>
      divorceSelectYearForm.form
        .bindFromRequest()
        .fold(
          formWithErrors => Future.successful(BadRequest(divorceSelectYearV(formWithErrors))),
          divorceDate =>
